...
void BLOCKCHAIN_SRV_PTCL_routing(struct mg_connection *c, int ev, void *ev_data, void *fn_data)
{
    ...
    switch (ev) {
        ...
        case MG_EV_READ:
        {
            // Search the dictionary of routes for the client's request
            response = (route = SERVER_ROUTES.search(&SERVER_ROUTES, c->recv.buf, c->recv.len)) ? 1:0;
            status = (response ? BLOCKCHAIN_SERVER_EXCHANGE:BLOCKCHAIN_SERVER_CLOSING);
            // Send 1 if the route is found or 0 if not
        ...
        case MG_EV_WRITE:
        {
            ...
            char msg[1024] = {0};
            // Prepare the message
            sprintf(msg, SERVER_LOGGING_FORMAT, uuid, c->peer.ip, c->peer.ip6, c->peer.is_ip6, c->peer.port, status, ev, "N/A", response, "BLOCKCHAIN SERVER: Route requested\n");
            // Log the message
            byte digest[64] = {0};
